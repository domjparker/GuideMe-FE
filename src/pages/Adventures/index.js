...
function Adventures() {
    ...
    const [modalCreateReview, setModalReview] = useState({ visible: false, id: '', hostId:'' })
    ...
    const createReviewClick = (id, host) => {
        setModalReview({visible:true, id:id, hostId:host});
        setChange(!change)
    }
                    ...
                    <Gridx classes={'grid-margin-x grid-margin-y noAdventures'}>
                        ...
                        {(adventures.length) ? adventures.map(adventure =>
                            <Cell key={adventure.hostId + " " + adventure._id} size={'medium-6 large-4'}>
                                ...
                                {loginState.loggedIn &&  <Btn className="reviewBtn" icon={<i className="fas fa-comments"></i>} classes={'button expanded'} handleClick={(e) => {e.stopPropagation(); createReviewClick(adventure._id, adventure.hostId)}} text={'Review this adventure'} />}
                            </Cell>
                         ) : <h3 className="noAdventures" style={{marginTop:"2vh"}}>I can't find any adventures meeting those search terms, please try again</h3>}
                    </Gridx>

                    {/* Modal lives here */}
                    <Review show={modalCreateReview.visible} handleModalClose={handleModalCreateReviewClose} id={modalCreateReview.id} hostId={modalCreateReview.hostId} />
